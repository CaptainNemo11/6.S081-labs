...
void 
vmprint_helper(pagetable_t p, int level)
{
  pte_t pte;
  pagetable_t child;
  if (level == 3)
...
// proc kernel pagetable map
void 
proc_kvmmap(pagetable_t p, uint64 va, uint64 pa, int sz, int perm)
{
...
void 
proc_kvmfree(pagetable_t p)
{
  pagetable_t page0_pa = (pagetable_t)PTE2PA(p[0]);
  pte_t p0_level1;
  for (int i = 0; i < 512; i++)
  {
    p0_level1 = page0_pa[i];
    if (p0_level1 & PTE_V)
    {
      kfree((void *)(pagetable_t)(PTE2PA(p0_level1)));
      page0_pa[i] = 0;
    }
  }
  kfree((void *)page0_pa);
